...
public class BoardDAO {
	...
	public List<BoardVO> getArticles(int start, int end){
		...
		try {
			conn = ConnUtil.getConnection();
			pstmt = conn.prepareStatement("select * from (select rownum rnum, num, writer, email, subject, pass, regdate, readcount, ref, step, depth, content, ip from (select * from board order by ref desc, step asc)) where rnum >=? and rnum<=?"); //���� <3>
			...
			if (rs.next()) {
				...
				do {
					...
					article.setEmail(rs.getNString("email"));
